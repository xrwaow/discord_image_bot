...
from vars import ACTIVE_LORAS, ACTIVE_MODEL
...
from comfy.utils import set_progress_bar_global_hook  # type: ignore
from comfy_extras.nodes_sd3 import (
    EmptySD3LatentImage as EmptySD3LatentImageClass,  # type: ignore
)
...
def _resolve_checkpoint() -> str:
    candidate = os.path.basename(ACTIVE_MODEL.model_path)
    try:
        folder_paths.get_full_path("checkpoints", candidate)
        return candidate
    except Exception:
        return ACTIVE_MODEL.model_path or candidate


def _apply_loras(model, clip, lora_keys: Iterable[str]):
    for key in lora_keys:
        lora_cfg = ACTIVE_LORAS.get(key)
        if not lora_cfg:
            continue
        for entry in lora_cfg.loras:
            path = entry.get("path")
            if not path:
                continue
            strength = float(entry.get("strength", 1.0))
            model, clip = LoraLoader.load_lora(model, clip, path, strength, strength)
    return model, clip


def _decoded_batch_to_pil(decoded_batch: torch.Tensor) -> List[Image.Image]:
    batch_np = np.clip(decoded_batch.detach().cpu().numpy(), 0.0, 1.0)
    return [Image.fromarray(sample) for sample in (batch_np * 255).astype(np.uint8)]


def _pil_to_tensor(image: Image.Image) -> torch.Tensor:
    return torch.from_numpy(
        np.expand_dims(np.array(image.convert("RGB"), dtype=np.float32) / 255.0, axis=0)
    )
...
def preprocess_gen_args(gen_args, default_args):
    ...
    if "lora" in gen_args:
        filtered = tuple(
            ...
            for name in (
                list(gen_args["lora"])
                if isinstance(gen_args["lora"], (list, tuple))
                else [gen_args["lora"]]
            )
            if name
        )
...
def _prepare_model(lora_keys: Optional[List[str]]):
    if ACTIVE_MODEL.clip_path and ACTIVE_MODEL.vae_path:
        model = UNETLoader.load_unet(ACTIVE_MODEL.model_path, "default")[0]
        clip_type = "krea2" if ACTIVE_MODEL.arch == "krea2" else "stable_diffusion"
        clip = CLIPLoader.load_clip(ACTIVE_MODEL.clip_path, clip_type, "default")[0]
        vae = VAELoader.load_vae(ACTIVE_MODEL.vae_path)[0]
    else:
...
def generate_images(gen_args):
    ...
    sampler_name, scheduler = (
        gen_args.get("sampler_name", "euler"),
        gen_args.get("scheduler", "normal"),
    )
    ...
    with torch.inference_mode():
        ...
        if ACTIVE_MODEL.arch == "zimg":
            latent = EmptySD3LatentImage.generate(
                gen_args["width"], gen_args["height"], batch_size
            )[0]
        else:
            latent = EmptyLatentImage.generate(
                gen_args["width"], gen_args["height"], batch_size=batch_size
            )[0]
        ...
        return _decoded_batch_to_pil(VAEDecode.decode(vae, sampled)[0])[:batch_size]
...
def img2img(image, gen_args):
    ...
    with torch.inference_mode():
        ...
        return _decoded_batch_to_pil(VAEDecode.decode(vae, sampled)[0])[0]


def upscale_image(image, gen_args):
    upscaled_width, upscaled_height = (
        int(image.size[0] * gen_args["scale"]),
        int(image.size[1] * gen_args["scale"]),
    )
    upscaled_width, upscaled_height = (
    ...
    return img2img(
        image.resize((upscaled_width, upscaled_height), Image.LANCZOS), gen_args
    )
